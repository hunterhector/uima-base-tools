...
public class ChineseStringUtils {
  ...
  public static boolean validate(Logger logger){
    logger.info("Using custom synchronized separatePuncs functions.");
    logger.info("Make sure we are using a customized ChineseStringUtils for thread-safe.");
    return true;
  }
  ...
  private synchronized static String separatePuncs(Character[] puncs_in, String ans) {
    /* make sure some punctuations will only appeared as one word (segmented from others). */
    /* These punctuations are derived directly from the training set. */
